...
const CharacterForm = ({ onSubmit, editingCharacter }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-2 p-4 border rounded">
      <input type="text" placeholder="Nom" value={name} onChange={(e) => setName(e.target.value)} className="border p-1 w-full" />
      <input type="text" placeholder="Nom réel" value={realName} onChange={(e) => setRealName(e.target.value)} className="border p-1 w-full" />
      <input type="text" placeholder="Univers" value={universe} onChange={(e) => setUniverse(e.target.value)} className="border p-1 w-full" />
